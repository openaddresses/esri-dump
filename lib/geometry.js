...
Downloader.prototype._read = function () {
    ...
    var queryString = {
        //Some ESRI servers except the geometry to be double encoded, other don't (will try both)
        ...
        request.get({url: fullUrl, qs: queryString, json: true}, function (error, response, data) {
            if (error || response.statusCode !== 200) {
                ...
                if (data.error.code === 400) {
                    queryString.geometry = [bounds.xmin, bounds.ymin, bounds.xmax, bounds.ymax].join(',');
                    return getQuery();
                }
